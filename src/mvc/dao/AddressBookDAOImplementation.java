    ...
    {   
       try
       {
          conn = DBConnection.getInstance().getConnect();
           ...
           String qry;
           qry = "insert into AddressBook values ('"+person.getName()+"','"+person.getMob()+"','"+person.getEmail()+"','"+person.getdob()+"')";
    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
            String qry="update AddressBook set name='"+person.getName()+"',mob='"+person.getMob()+"',email='"+person.getEmail()+"',dob='"+person.getdob()+"' where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
        ...
        try {
            ...
            while(rs.next()){
             ...
             detailPanel.setMobile(rs.getString("mob"));
              detailPanel.setdob(rs.getString("dob"));
           }
            conn.close();
        }
         catch (SQLException ex) {
